...
public:
	AddEdge(std::shared_ptr<IRModel> irModelGui, std::string from_layer, std::string from_port, std::string to_layer, std::string to_port) : 
		irModelGui(irModelGui), from_layer(from_layer), from_port(from_port), to_layer(to_layer), to_port(to_port) {}
	AddEdge(std::shared_ptr<IRModel> irModelGui, std::shared_ptr<OutputPort> outputPort, std::shared_ptr<InputPort> inputPort) :
